...
class AppEncryptionAES: AppEncryption {
    ...
    override fun encrypt(plainText: String): String {
        ...
        return Base64.getMimeEncoder().encodeToString(encryptedBytes) + ":" + Base64.getEncoder().encodeToString(iv)
    }

    override fun decrypt(encryptedText: String): String {
        val parts = encryptedText.split(":")
        val encryptedData = Base64.getMimeDecoder().decode(parts[0])
        val iv = Base64.getMimeDecoder().decode(parts[1])
        val cipher = Cipher.getInstance("AES/GCM/NoPadding")
        val decodedKey = Base64.getMimeDecoder().decode(key)
        val secretKey: SecretKey = SecretKeySpec(decodedKey, "AES")
